...
# Install CASCC 
## install R and R package using anaconda (recommended)
```{r, eval = FALSE}
...
checkDependencies() # We highly recommend users to use Seurat V4 instead of V5 as dependency of CASCC. 
```
...
df[1:10, ] # from left to right, they represent macrophage cell, Ductal cell type 1, B cell, Endotehlial, Ductal cell type 2, T cell, Stellate cell and Fibroblast cell.
```
...
### Attractor plot  
Color the cells by the expression of the top 5 genes of each attractor. 
```{r, fig.width=8, fig.height=6}
attractorPlot(adata, attrs[1:4], n.col = 2)
```

### Understand CASCC output
CASCC output object:
...
## Quick Start - find co-expression signatures 
...
attr.res <- findAttractor.adaptive(Data_PDAC_peng_2k, "LUM")

# co-expression attractor 
head(attr.res$attractor.final)	
```
...
Generate the toy data for the quick start. 
...
```
## Run CASCC within Seurat workflow
```{r, eval = FALSE}
## after processed Seurat workflow
# Seurat V4 is highly recommended!
## Users' SeuratObject 
SeuratVersionCheck = as.numeric(strsplit(as.character(packageVersion("Seurat")), "\\.")[[1]][1])
if (SeuratVersionCheck == 4) {
